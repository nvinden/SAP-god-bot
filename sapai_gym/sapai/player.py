...
# Player does not have enough gold to perform action
...
class Player:
    ...
    def buy_pet(self, pet):
        ...
        if len(self.team) == self._max_team:
            raise FullTeamException("Attempted to buy Pet on full team")
    ...
    def sell(self, pet):
        ...
        if pet.obj.name == "pet-none":
            raise WrongObjectException(f"Attempted to sell an empty spot")

        if isinstance(pet, TeamSlot):
            pet = pet._pet

        if type(pet).__name__ != "Pet":
            raise WrongObjectException(f"Attempted to sell Object {pet}")
